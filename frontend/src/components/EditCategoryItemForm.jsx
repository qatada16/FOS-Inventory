...
export const EditCategoryItemForm = ({ item, categoryId, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        // Update the item in the dynamic table
        const response = await axios.put(
        `${import.meta.env.VITE_API_BASE_URL}/api/categories/${categoryId}/items/${item.id}`,
        {
            name: formData.name,
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="relative max-w-md w-full rounded-xl overflow-hidden bg-gradient-to-br from-[#1a2f36] to-[#284952] border border-[#95CFAB]/20 p-6">
        ...
        <h2 className="text-2xl font-bold text-[#95CFAB] mb-6">Edit Item</h2>
